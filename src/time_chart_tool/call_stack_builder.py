...
from .utils.event_utils import remove_triton_suffix, create_event_id
...
class CallStackNode:
    ...
    def __init__(self, event: ActivityEvent, parent: Optional['CallStackNode'] = None):
        ...
        # 为快速查找创建事件标识符
        self.event_id = create_event_id(event)
...
def _group_events_by_pid_tid(events: List[ActivityEvent]) -> Dict[Tuple[int, int], List[ActivityEvent]]:
    """
    ...
    print(f"按pid/tid分组完成，共 {len(events_by_pid_tid)} 个组")
    return dict(events_by_pid_tid)


def _contains(parent_event: ActivityEvent, child_event: ActivityEvent) -> bool:
    """
    检查父事件是否包含子事件
...
def _find_parent_node(interval: EventInterval, active_stack: List[CallStackNode], segment_tree: SegmentTree) -> CallStackNode:
    """
    为当前区间找到合适的父节点
    ...
    for i in range(len(active_stack) - 1, -1, -1):
        ...
        # 检查候选节点是否包含当前区间
        if _contains(candidate_event, interval.event):
            return candidate
...
def _process_interval(interval: EventInterval, active_stack: List[CallStackNode], segment_tree: SegmentTree):
    """
    处理单个事件区间
    ...
    # 2. 查找当前区间的父节点
    parent = _find_parent_node(interval, active_stack, segment_tree)
...
def _sweep_line_algorithm(intervals: List[EventInterval], segment_tree: SegmentTree) -> CallStackNode:
    """
    扫描线算法构建调用栈树
    ...
    for interval in intervals:
        # 处理当前区间
        _process_interval(interval, active_stack, segment_tree)
    
    return root


def _build_call_stack_tree(events: List[ActivityEvent]) -> Optional[CallStackNode]:
    """
    使用扫描线算法构建调用栈树
    ...
    # 4. 使用扫描线算法构建调用栈树
    return _sweep_line_algorithm(intervals, segment_tree)


def build_call_stack_trees(events: List[ActivityEvent]) -> Dict[Tuple[int, int], CallStackNode]:
    """
    为所有事件构建调用栈树（纯函数）
    ...
    """
    # 1. 按pid和tid分组事件
    events_by_pid_tid = _group_events_by_pid_tid(events)
    
    # 2. 为每个(pid, tid)组构建调用栈树
    call_stack_trees = {}
    
    for (pid, tid), group_events in events_by_pid_tid.items():
        print(f"为进程 {pid} 线程 {tid} 构建调用栈，事件数: {len(group_events)}")
        
        # 过滤掉没有duration的事件
        valid_events = [e for e in group_events if e.dur is not None and e.dur > 0]
        
        if not valid_events:
            logger.warning(f"进程 {pid} 线程 {tid} 没有有效的事件")
            continue
        
        # 构建调用栈树
        root = _build_call_stack_tree(valid_events)
        if root:
            call_stack_trees[(pid, tid)] = root
    
    print(f"成功构建 {len(call_stack_trees)} 个调用栈树")
    return call_stack_trees

class CallStackBuilder:
    """调用栈构建器（兼容类）"""
    
    def build(self, events: List[ActivityEvent]) -> Dict[Tuple[int, int], CallStackNode]:
        """
        构建调用栈树
        ...
        Returns:
            Dict[Tuple[int, int], CallStackNode]: 调用栈树字典
        """
        return build_call_stack_trees(events)

# Alias for compatibility if needed, but prefer build_call_stack_trees
build_call_stacks = build_call_stack_trees
