...
class DataPresenter:
    ...
    def _present_single_file(self, data: Dict[Union[str, tuple], AggregatedData], 
        ...
        for key, aggregated_data in data.items():
            ...
            if show_stream:
                ...
                streams = set()
                for cpu_event in aggregated_data.cpu_events:
                    if cpu_event.stream_id is not None:
                        streams.add(str(cpu_event.stream_id))
                for kernel_event in aggregated_data.kernel_events:
                    if kernel_event.stream_id is not None:
                        streams.add(str(kernel_event.stream_id))
                row['streams'] = '\n'.join(sorted(streams)) if streams else ''
    ...
    def _present_multiple_files(self, data: Dict[str, Any], 
            ...
            if compare_dtype or compare_shape or compare_name or show_kernel_duration:
                ...
                if len(file_labels_list) >= 2:
                    ...
                    if show_kernel_duration:
                        ...
                        # 添加 total duration 差值比较
                        total_duration1 = row.get(f'{label1}_kernel_total_duration', 0.0)
                        total_duration2 = row.get(f'{label2}_kernel_total_duration', 0.0)
                        
                        # 计算差值：前一个label的total duration - 当前label的total duration
                        duration_diff = total_duration1 - total_duration2
                        row[f'{label1}_vs_{label2}_total_duration_diff'] = duration_diff
        ...
        # 生成输出文件
        base_name = self._generate_base_name(aggregation_spec, show_dtype, show_shape, show_kernel_names, show_kernel_duration, show_timestamp, show_name, show_call_stack, show_stream, file_labels, include_op_patterns, exclude_op_patterns, include_kernel_patterns, exclude_kernel_patterns)
    ...
    def _generate_base_name(self, aggregation_spec: str, show_dtype: bool, show_shape: bool, 
                           ...
                           show_name: bool, show_call_stack: bool = False, show_stream: bool = False, file_labels: List[str] = None,
